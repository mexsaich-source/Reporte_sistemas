...
const UserRoleBadge = ({ role }) => {
    const config = {
        'Administrador': { icon: ShieldCheck, color: 'text-purple-700 bg-purple-50 border-purple-200' },
        'Técnico': { icon: Shield, color: 'text-blue-700 bg-blue-50 border-blue-200' },
        'Usuario': { icon: UserCheck, color: 'text-slate-600 bg-slate-100 border-slate-200' }
    };
            ...
            <div className={`fixed top-0 right-0 h-full w-full sm:w-[450px] bg-white shadow-2xl z-50 flex flex-col animate-in slide-in-from-right duration-300`}>
                <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <div>
                        ...
                        <h2 className="text-xl font-black text-slate-900">{user?.id}</h2>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-8 text-sm">
                    ...
                    <div className="flex items-center gap-5">
                        <div className="relative">
                            <img
                                ...
                                className="w-20 h-20 rounded-[2rem] border-4 border-white shadow-lg shadow-slate-200/50"
                                alt="Profile"
                            />
                            <div className={`absolute bottom-0 right-0 w-5 h-5 rounded-full border-4 border-white ${user?.status === 'Active' ? 'bg-emerald-500' : 'bg-slate-400'}`}></div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:bg-white hover:border-slate-200 transition-colors">
                            <div className="bg-white p-2 rounded-xl shadow-sm"><Mail size={16} className="text-slate-400" /></div>
                            <div>
                                <span className="block text-[10px] font-black uppercase tracking-widest text-slate-400">Correo Electrónico</span>
                                <span className="font-semibold text-slate-700">{user?.email}</span>
                            </div>
                        </div>
                        <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:bg-white hover:border-slate-200 transition-colors">
                            <div className="bg-white p-2 rounded-xl shadow-sm"><Users size={16} className="text-indigo-400" /></div>
                            <div>
                                <span className="block text-[10px] font-black uppercase tracking-widest text-slate-400">Departamento</span>
                                <span className="font-bold text-indigo-700">{user?.department}</span>
                            </div>
                        </div>
                        <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:bg-white hover:border-slate-200 transition-colors">
                            <div className="bg-white p-2 rounded-xl shadow-sm"><Activity size={16} className="text-emerald-400" /></div>
                            <div>
                                <span className="block text-[10px] font-black uppercase tracking-widest text-slate-400">Última Conexión</span>
                                <span className="font-semibold text-slate-700">{user?.lastLogin}</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl shadow-sm">
                                <div className="flex flex-col">
                                    <span className="font-bold text-slate-800">Laptop Dell Latitude 7420</span>
                                    <span className="text-xs text-slate-400 font-medium">DEV-8920</span>
                                </div>
                                <ChevronRight size={16} className="text-slate-300" />
                            </div>
                            <div className="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl shadow-sm">
                                <div className="flex flex-col">
                                    <span className="font-bold text-slate-800">Monitor Dell 27"</span>
                                    <span className="text-xs text-slate-400 font-medium">DEV-8921</span>
                                </div>
                                <ChevronRight size={16} className="text-slate-300" />
                            </div>
                        </div>
            ...
            <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden">
                <div className="p-8 flex flex-col justify-between items-start gap-6 border-b border-slate-100/60 bg-slate-50/30">
                    <div className="w-full flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div>
                            <h3 className="text-xl font-black text-slate-900 tracking-tight">Directorio del Personal</h3>
                            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Control de Accesos y Empleados</p>
                        </div>
                        <div className="flex gap-3 w-full md:w-auto">
                            <div className="flex-1 md:w-72 flex items-center gap-3 text-slate-400 bg-white px-4 py-2.5 rounded-2xl border border-slate-200 shadow-sm focus-within:border-blue-500 focus-within:ring-4 focus-within:ring-blue-500/10 transition-all group">
                    ...
                    <table className="w-full text-left border-collapse whitespace-nowrap">
                        <thead>
                            <tr className="text-slate-400 text-[10px] uppercase tracking-[0.2em] font-black">
                                <th className="p-4 pl-6 pb-6 border-b border-slate-100/50">Empleado</th>
                                <th className="p-4 pb-6 border-b border-slate-100/50">Contacto</th>
                                <th className="p-4 pb-6 border-b border-slate-100/50">Rol en Sistema</th>
                                <th className="p-4 pb-6 border-b border-slate-100/50">Estado</th>
                                <th className="p-4 pr-6 pb-6 border-b border-slate-100/50 text-right">Último Acceso</th>
                            </tr>
                        </thead>
                        <tbody className="text-sm">
                            {usersData.map((user) => (
                                <tr key={user.id} onClick={() => setSelectedUser(user)} className="group transition-all duration-300 hover:bg-slate-50/80 cursor-pointer">
                                    <td className="p-4 pl-6">
                                        <div className="flex items-center gap-4">
                                            <img
                                                src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=f1f5f9&color=0f172a&bold=true`}
                                                className="w-10 h-10 rounded-2xl border border-slate-200 group-hover:scale-110 transition-transform"
                                                alt={user.name}
                                            />
                                            <div className="flex flex-col">
                                                <span className="font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{user.name}</span>
                                                <span className="font-bold text-[10px] text-indigo-600 uppercase tracking-widest">{user.department}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="p-4">
