-- SQL MANUAL (BOOTSTRAP): Configuracion omnicanal por area (IT/ING)
-- Usa este script en SQL Editor si aun no aplicaste la migracion.
...
CREATE OR REPLACE FUNCTION public.get_my_notification_area_settings()
RETURNS TABLE (
  area TEXT,
  telegram_bot_token TEXT,
  smtp_host TEXT,
...
BEGIN
  ...
  RETURN QUERY
  SELECT s.area, s.telegram_bot_token, s.smtp_host, s.smtp_port, s.smtp_user, s.smtp_pass,
         s.smtp_from_name, s.meta_access_token, s.meta_phone_number_id, s.updated_at
  FROM public.notification_area_settings s
  WHERE s.area = v_area;
END;
$$;
...
REVOKE ALL ON FUNCTION public.get_my_notification_area_settings() FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION public.get_my_notification_area_settings() TO authenticated;
